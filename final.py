...
print("\nOptimized Portfolio (Machine Learning Approach):")
print(f"Return: {p_returns_ml}")
print(f"Volatility: {p_volatility_ml}")
print(f"Sharpe Ratio: {sharpe_ratio_ml}")
...
# Step 6: Portfolio Optimization with Minimum Variance Model
def portfolio_volatility(weights, cov_matrix):
    volatility = np.sqrt(np.dot(weights.T, np.dot(cov_matrix * 252, weights)))
    return volatility

# Constraints and bounds
constraints_min_var = ({'type': 'eq', 'fun': check_sum})
bounds = tuple((0, 1) for _ in range(len(mean_returns)))
...
optimized_weights_min_var = optimized_min_var.x

# Display the optimized portfolio (Minimum Variance Approach)
min_var_volatility = portfolio_volatility(optimized_weights_min_var, cov_matrix)
min_var_return = np.sum(mean_returns * optimized_weights_min_var) * 252

print("\nOptimized Portfolio (Minimum Variance Approach):")
print(f"Return: {min_var_return}")
print(f"Volatility: {min_var_volatility}")

# Print the optimal weights in terms of percentages, excluding zero allocations
print("Optimal Weights (Percentages):")
for i, col in enumerate(all_data.columns):
    if optimized_weights_min_var[i] > 0:
        print(f"{col}: {optimized_weights_min_var[i] * 100:.2f}%")

# Simulate random portfolios (optional)
